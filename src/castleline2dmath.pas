...
{*
 Модуль содержит некоторые математические методы, а так же методы для построения
...
uses
  ...
  procedure AddArcBetweenPoints(var Arc:TVector2List; const Center,O,A,B:TVector2; const EdgesCount:integer);
  ...
  0 точка это A.Y-width/2, 1 это A.Y+width/2,  ....  @br
  width - высота (или ширина) прямоугольника
  }
  ...
  procedure AddCrossSegments(var Arc:TVector2List; const O,A1,A2,B1,B2:TVector2);

  {*
  Сортирует вершины треугольника в порядок обхода по часовой стрелке
  в зависимости от Scale (во 2 и 4 четвертях порядок обхода меняется)
  }
  procedure DoGoodTriangle(var Lt:TVector2List; A,B,C:integer; Scale:TVector2);
...
begin
 if (A1-B1).IsZero and (A2-B2).IsZero then Exit;
